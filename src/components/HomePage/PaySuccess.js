...
function PaySuccess() {
  ...
  useEffect(() => {
    if (searchParams.get('success')===true && searchParams.get('key')/*  eventInfo?.[0] */ && user?.uid) {
    ...
    {searchParams.get('success') === true ? (
      ...
      <>
        <div>
          ...
          <button onClick={()=> navigateTo(`/eventpage/${searchParams.get("key")}`)}>Back to Event! Try Again!</button>
        </div>
